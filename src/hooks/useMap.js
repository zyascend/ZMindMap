...
export class TreeDataCreater {
  ...
  pathData(data) {
    ...
    const { x, y, contentWidth, rectHeight, depth } = data.source
    const bézierCurveGenerator = linkHorizontal()
      .x(d => d.x)
      .y(d => d.y)
    ...
    if (depth === 0) {
      ...
      bezierLine = bézierCurveGenerator({
        source: horiEnd,
        target: {
          x: data.target.x - this.borderXPadding,
          y:
            data.target.y -
            data.target.rectHeight / 2 -
            this.offsetBottom -
            this.borderYPadding
        }
      })
    } else if (depth === 1) {
      const end = { x: data.target.x, y: data.target.y }
      const horiStart = {
        x: x + contentWidth + this.borderXPadding,
        y: y - rectHeight / 2 - this.borderYPadding - this.offsetBottom
      }
      ...
      bottomLineLeaf = `M${end.x} ${end.y}L${
        end.x + data.target.contentWidth + this.borderXPadding
      } ${end.y}`
    } else {
      ...
      const bezierEnd = { x: data.target.x, y: data.target.y }
      bottomLine = `M${horiStart.x} ${horiStart.y}L${horiEnd.x} ${horiEnd.y}`
      bezierLine = bézierCurveGenerator({
        source: horiEnd,
        target: bezierEnd
      })
      bottomLineLeaf = `M${bezierEnd.x} ${bezierEnd.y}L${
        bezierEnd.x + data.target.contentWidth + this.borderXPadding
      } ${bezierEnd.y}`
    }
    return {
      data: `${bottomLine}${bezierLine}${bottomLineLeaf}`,
      id: `${data.source.id}|${data.target.id}`
    }
  }
  ...
  calculateXY(root) {
    ...
    root.eachAfter(node => {
      ...
      if (children) {
        node.y = meanY(children)
        const extraY =
          node.children[0].y - this.gapY - (node.y - node.rectHeight)
        if (extraY > 0) {
          node.each(n => {
            n.y += extraY
          })
        ...
        //   : 30 + rectHeight
        if (preNode) {
          node.y = preNode.y + rectHeight + this.gapY
        } else if (!node.y) {
          node.y = 30 + rectHeight
        }
        preNode = node
  ...
  measureContent(root) {
    ...
    const f = this.measureSvg
      .append('foreignObject')
      .attr('width', this.maxNodeWidth)
    // 根节点字大一点
    ...
    f.append('xhtml:div')
      .attr(
        'style',
        `word-break:normal; width:auto;width:fit-content;display:block;white-space:pre-wrap;word-wrap:break-word;overflow:hidden;font-size:${fontSize}px;line-height:${lineHeight}px;`
      )
      .text(root.data.html)
  ...
  measureWidthAndHeight(root) {
    ...
    root.collapseY =
      root.depth > 1
        ? root.contentHeight + this.offsetBottom
        : (root.contentHeight - this.btnSize) / 2
    // 3.2 添加按钮位置
